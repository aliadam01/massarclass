...
//                        }
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
      ...
      if(  $loaded !== $dom->loadHTML(mb_convert_encoding($this->securityScanFile($pFilename), 'HTML-ENTITIES', 'UTF-8'))){unlink($_SESSION['الملف']); exit();}
        $loaded = $dom->loadHTML(mb_convert_encoding($this->securityScanFile($pFilename), 'HTML-ENTITIES', 'UTF-8'));
		
		//if(  $loaded !== $dom->loadHTML(mb_convert_encoding($this->securityScanFile($pFilename), 'HTML-ENTITIES', 'UTF-8'))){header('location: Not excel massar.php'); exit();}
		if ($loaded === false) {
